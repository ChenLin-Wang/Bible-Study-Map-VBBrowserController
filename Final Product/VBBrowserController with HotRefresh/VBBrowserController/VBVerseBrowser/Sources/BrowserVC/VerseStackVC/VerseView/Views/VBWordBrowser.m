...
@interface VBWordBrowser() {
    __weak CDSQLite3OperaterDelegate * DBOperator;
    __weak VBVerseReference * verseRef;
}
...
// ----- DataWrapper 的所需参数 -----
@property NSOperationQueue * wordFetchingQueue;
@property NSOperationQueue * commentFetchingQueue;
// --------------------------------------------------

// ----- ContentSize 的所需参数 -----
@property NSSize contentSize;
@property (class, readonly) NSSize minSize;
@property (class, readonly) CGFloat widthOverage;
@property (class, readonly) CGFloat heightOverage;
...
- (void)prepareForDataWrapper;

- (void)fetchWordCardsFromDatabaseWithCompleteBlock:(void (^)(void))completeBlock;
- (void)fetchCommentCardsFromDatabaseWithCompleteBlock:(void (^)(void))completeBlock;
...
+ (instancetype)newBrowserWithDBOperator:(CDSQLite3OperaterDelegate *)dbOperator layoutDelegate:(nonnull id<VBVerseViewLayoutDelegate>)layoutDelegate {
    VBWordBrowser * wordBrowser = [VBWordBrowser new];
    wordBrowser->DBOperator = dbOperator;
    wordBrowser.layoutDelegate = layoutDelegate;
    [wordBrowser prepare];
    return wordBrowser;
}
...
- (void)refreshWithVerseRef:(VBVerseReference *)verseRef {
    ...
    [self clearCurCards];
    [self deployWordGroupsWithCompleteBlock:^{
        [self deployCommentCardsWithCompleteBlock:^{
...
- (void)deployWordGroupsWithCompleteBlock:(void (^)(void))completeBlock {
    [self fetchWordCardsFromDatabaseWithCompleteBlock:^{
...
- (void)deployCommentCardsWithCompleteBlock:(void (^)(void))completeBlock {
    [self fetchCommentCardsFromDatabaseWithCompleteBlock:^{
...
+ (NSSize)minimumSize { return NSMakeSize(self.minSize.width + self.widthOverage, self.minSize.height + self.heightOverage); }
+ (NSSize)minSize { return NSMakeSize(VBWordCard.fixedSize.width, VBWordCard.fixedSize.height * 3 + VB_CARD_SPACING * 2); }
+ (CGFloat)widthOverage { return VBWordCard.fixedSize.width + VB_CARD_SPACING; }
+ (CGFloat)heightOverage { return VB_SPACING_OVERAGE; }
- (CDSQLite3OperaterDelegate *)DBOperator { return self->DBOperator; }
- (VBVerseReference *)verseRef { return self->verseRef; }
...
- (void)prepareForDataWrapper {
//    self.wordFetchingQueue = [NSOperationQueue new];
//    self.commentFetchingQueue = [NSOperationQueue new];
}

- (void)fetchWordCardsFromDatabaseWithCompleteBlock:(void (^)(void))completeBlock {
//    [self.wordFetchingQueue addOperationWithBlock:^{
    [self wordsFromDatabaseWithCompleteBlock:^(NSArray<VBWord *> * _Nonnull words) {
        [self.wordGroupController clearAllGroups];
        [NSOperationQueue.mainQueue addOperationWithBlock:^{
            [self.wordGroupController remakeWithWords:words cardDelegate:self];
            completeBlock();
        }];
    }];
//    }];
...
//    [self.commentFetchingQueue addOperationWithBlock:^{
    
    [self commentsFromDatabaseWithCompleteBlock:^(NSArray<VBComment *> * _Nonnull comments) {
...
#import "VBVerseViewLayoutDelegate.h"
...
- (void)calculateTotalContentSizeWithSendChangeNotification:(BOOL)sendChangeNotification{
    ...
    self.contentSize = NSMakeSize(self.contentSize.width < VBWordBrowser.minSize.width ? VBWordBrowser.minSize.width : self.contentSize.width, self.contentSize.height < VBWordBrowser.minSize.height ? VBWordBrowser.minSize.height : self.contentSize.height);
...
- (void)contentSizeChanged {
    NSSize size = NSMakeSize(self.contentSize.width + VBWordBrowser.widthOverage, self.contentSize.height + VBWordBrowser.heightOverage);
//    [CATransaction begin];
...
//    [CATransaction commit];
    [self.layoutDelegate reLayoutWithContentSize:size verseVC:(VBVerseViewController *)self];
}
