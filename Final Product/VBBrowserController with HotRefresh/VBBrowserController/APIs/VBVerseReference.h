...
/**
 * @brief 这个方法用于判断两个 VerseReference 是否连续
 * 实现此方法，并添加判断。VBBrowserController 模块将调用此方法来确认传入的 verseReference 是连续的
 *
 * @return 返回 VBVerseRefCompareResultAscending 表明传入的 verseRef 在右方。
 * 否则，返回 VBVerseRefCompareResultDescending。
 * 若返回值为 VBVerseRefCompareResultNotContinuously 表示这两个 verseRef 不连续
 */
