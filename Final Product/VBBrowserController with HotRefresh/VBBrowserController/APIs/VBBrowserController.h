...
/**
 * @brief 当版本被切换(即当用户通过点击界面上蓝色的版本切换 view 的两个按钮切换经文版本时)，browser 将通过此协议方法向 dataSource 发送通知
 *
 * @param browser 表示请求者
 * @param verseReference 表示发生改变的经文
 * @param newBibleVersion 更改后的版本字符串
 */
- (void)bibleVersionDidChange:(VBBrowserController *)browser
               verseReference:(VBVerseReference *)verseReference
              newBibleVersion:(NSString *)newBibleVersion;


/**
 * @brief 当某条经文的翻译 TextField 内容被改变后，browser 通过此协议方法向 dataSource 发送通知
 *
 * @param browser 表示请求者
 * @param verseReference 表示发生改变的经文
 * @param bibleVersion 该经文当前的版本字符串
 * @param newTranslationContent 新输入的翻译内容
 */
- (void)translationShouldChange:(VBBrowserController *)browser
                 verseReference:(VBVerseReference *)verseReference
                   bibleVersion:(NSString *)bibleVersion
          newTranslationContent:(NSString *)newTranslationContent;
 ...
 * @warning verseReferences 必须传入连续经文，否则程序会抛出异常，另外，请至少传入 2 个 verseReferences，否则程序会不可避免地崩溃
 */
