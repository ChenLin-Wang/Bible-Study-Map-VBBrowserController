...
// 这个测试方法中的所有数据都是随便生成的，VBBrowserController 不检测这些经文是否是连续的。所以，无论经文是否连续，此模块都显示。
- (void)test {
...
// 务必在 verseReference 到头时返回 NULL，否则程序将陷入死循环
- (VBVerseReference *)verseWillAppend:(VBBrowserController *)browser currentVerseReference:(VBVerseReference *)curVerseReference requestType:(VBVerseRequestType)requestType {
    ...
    if (curVerseReference.verseIndex.integerValue >= 30) return NULL;
